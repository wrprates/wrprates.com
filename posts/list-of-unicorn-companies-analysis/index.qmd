...
unicorn_data <- readxl::read_excel("unicorn_companies.xlsx", skip = 2) |>
  ...
  dplyr::mutate(
    ...
    Quartiles = dplyr::case_when(
      `Valuation ($B)` <= quantile(`Valuation ($B)`, 0.25, na.rm = TRUE) ~ 'Q1',
      `Valuation ($B)` <= quantile(`Valuation ($B)`, 0.5, na.rm = TRUE) ~ 'Q2',
      `Valuation ($B)` <= quantile(`Valuation ($B)`, 0.75, na.rm = TRUE) ~ 'Q3',
      `Valuation ($B)` <= quantile(`Valuation ($B)`, 1, na.rm = TRUE) ~ 'Q4'
    )#,
     # Quartiles = dplyr::mutate(dplyr::across(`Valuation ($B)`, .fn = function(x) cut(x, quantile(x, 0:4/4))))
  )
...
```{r}
tree_map_chart <- function(data, quartile, colors_df) {
  treemap_data <- data |>
    dplyr::arrange(dplyr::desc(`Valuation ($B)`)) |>
  ...
  min_valuation <- min(data$`Valuation ($B)`)
  max_valuation <- max(data$`Valuation ($B)`)

  treemap_data |>
    echarts4r::e_charts() |>
    echarts4r::e_title(
      text = paste0("Unicorns by Industry - from $",  min_valuation, "B to $", max_valuation, "B"),
      subtext = paste0("Quartile ", quartile)
    ) |>
    echarts4r::e_treemap(
      leafDepth = 1,
      #breadcrumb = TRUE,
...
tree_map_chart(data = unicorn_data, quartile = "Q4", colors_df = colors_df)
```
...
htmltools::div(
  tree_map_chart(data = unicorn_data, quartile = "Q4", colors_df = colors_df),
  tree_map_chart(data = unicorn_data, quartile = "Q3", colors_df = colors_df),
  tree_map_chart(data = unicorn_data, quartile = "Q2", colors_df = colors_df),
  tree_map_chart(data = unicorn_data, quartile = "Q1", colors_df = colors_df),
)
